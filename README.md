...
- **원리**: TF-IDF로 상위 100개 사례 추출 → 5개 그룹으로 분할 → 각 Group AI가 병렬 분석 → Head AI가 종합
...
- **원리**: Character n-gram(2~4글자 단위) 방식으로 복합어 정확 매칭
- **장점**: 형태소 분석기 대비 10배 이상 빠른 속도, 고성능 서버 불필요
- **예시**: "폴리우레탄폼" 검색 시
  - 기존 키워드 검색: 정확히 일치하는 단어만 검색
  - Character n-gram: "폴리", "리우", "우레", "레탄", "탄폼" 등으로 분해하여 유사 문서 검색
  - 결과: 띄어쓰기나 변형된 표현도 정확하게 검색
