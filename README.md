...
### 3. TF-IDF Character n-gram: 일반 노트북에서도 빠르게 구동

- **원리**: TF-IDF Character n-gram(2~4글자 단위) 방식으로 복합어 정확 매칭
- **장점**:
  - **임베딩보다 가볍고 빠릅니다**: 일반적인 AI 챗봇은 '임베딩'이라는 방식으로 문서를 검색합니다. 문장의 의미를 복잡한 숫자로 변환하는 과정인데, 시간이 오래 걸리고 고성능 서버가 필요합니다. 이 챗봇은 TF-IDF 방식을 사용합니다. 문서에서 중요한 단어를 빠르게 찾아내는 기술입니다. 임베딩보다 압도적으로 빠르며, 일반 노트북으로도 수천 건의 문서를 1~2초 만에 검색할 수 있습니다.
  - **TF-IDF 단어 방식보다 정확합니다**: 일반적인 TF-IDF는 '단어' 단위로 검색합니다. 문제는 띄어쓰기나 조사가 조금만 달라져도 다른 단어로 인식한다는 점입니다. 이 챗봇은 Character n-gram 방식을 추가했습니다. 단어를 2~4글자 단위로 잘게 쪼개서 검색합니다.
- **예시**: "폴리우레탄폼" 검색 시
...
#### 검색 팁
- 영문 키워드 사용 필수
- HS 코드나 참고문서번호가 가장 정확
- 짧고 일반적인 단어 사용 권장 (예: "ceramic mug" → "mug")
...
[사용자 질문]
    ↓
[TF-IDF Character n-gram 검색] → 상위 100개 유사 사례 추출
    ↓
[5개 그룹으로 분할] → 각 그룹 20개 사례
    ↓
...
#### 3. 하이브리드 RAG (TF-IDF + AI)

- **쉬운 설명**: TF-IDF로 빠르게 검색 → AI가 정확하게 답변
- **5단계 처리 과정**:
  1. **사용자 질문 입력**: 품목에 대한 질문 입력
  2. **TF-IDF 검색**: Character n-gram 방식으로 상위 100개 유사 사례 추출 (0.02초)
  3. **그룹 분할**: 100개 사례를 5개 그룹으로 분할 (각 그룹 20개)
  4. **병렬 AI 분석**: 5개 Group AI가 ThreadPoolExecutor로 동시 실행하여 각 그룹 분석
  5. **최종 종합**: Head AI가 5개 분석 결과를 통합하여 최종 전문가 답변 생성
- **장점**: 고성능 서버 불필요, 빠른 속도 (5-10초), 높은 정확도
