...
class HSDataManager:
    ...
    def generate_embeddings_batch(self, texts: List[str], batch_size: int = 50) -> List[np.ndarray]:
        """
        배치로 임베딩 생성 (Rate limit 준수)
        Args:
            texts: 임베딩을 생성할 텍스트 리스트
            batch_size: 배치 크기 (최대 100, 권장 50)
        Returns:
            임베딩 벡터 리스트
        """
        if not self.client:
        ...
        for i in range(0, len(texts), batch_size):
            ...
            try:
                ...
                # 실패 시 빈 벡터 추가
                for _ in batch:
                    embeddings.append(np.zeros(768))  # gemini-embedding-001은 768차원 (config로 설정)
    ...
    def initialize_all_embeddings(self, force_regenerate: bool = False):
        ...
        print("=" * 70)
        print("EMBEDDING INITIALIZATION START")
        print("=" * 70)
        print(f"Total items in database: {len(self.items_by_id)}")

        # 소스별 아이템 개수 통계
        source_counts = defaultdict(int)
        for item_id in self.items_by_id.keys():
        ...
        for source, count in sorted(source_counts.items()):
            ...
            if 'HS분류사례' in source or 'HS위원회' in source or 'HS협의회' in source:
                domestic_total += count
            else:
                overseas_total += count

        print(f"\nDomestic cases total: {domestic_total}")
        print(f"Overseas cases total: {overseas_total}")
        print(f"Grand total: {domestic_total + overseas_total}")
        ...
        print(f"Failed embeddings (zero vectors): {failed_embeddings}")
        print(f"Success rate: {successful_embeddings/len(embeddings)*100:.1f}%")

        # 소스별 임베딩 개수 확인
        print("\nEmbedding coverage by source:")
        source_embedding_counts = defaultdict(int)
        for item_id in self.embeddings_cache.keys():
            source = item_id.rsplit('_', 1)[0]
            source_embedding_counts[source] += 1

        for source, count in sorted(source_embedding_counts.items()):
            original_count = source_counts[source]
            coverage = count / original_count * 100 if original_count > 0 else 0
            status = "OK" if coverage == 100 else "INCOMPLETE"
            print(f"  [{status}] {source}: {count}/{original_count} ({coverage:.1f}%)")
    ...
    def search(self, query: str, max_results: int = 5) -> List[Dict[str, Any]]:
        ...
        """
        if len(self.embeddings_cache) > 0:
            print(f"[SEARCH] Using HYBRID search (keyword + embedding) for query: '{query[:50]}...'")
            print(f"[SEARCH] Embedding cache size: {len(self.embeddings_cache)} items")
            return self.search_hybrid(query, max_results=max_results)
        else:
            print(f"[SEARCH] Using KEYWORD-ONLY search for query: '{query[:50]}...'")
            print(f"[SEARCH] Warning: No embeddings cached. Semantic search unavailable.")
    ...
    def search_domestic_group(self, query: str, group_idx: int, max_results: int = 3) -> List[Dict[str, Any]]:
        ...
        if len(self.embeddings_cache) > 0:
            print(f"[DOMESTIC GROUP {group_idx+1}] Using HYBRID search on sources: {sources}")
            return self.search_hybrid(query, max_results=max_results, source_filter=sources)
        else:
            print(f"[DOMESTIC GROUP {group_idx+1}] Using KEYWORD search on sources: {sources}")
    ...
    def search_overseas_group(self, query: str, group_idx: int, max_results: int = 3) -> List[Dict[str, Any]]:
        ...
        if len(self.embeddings_cache) > 0:
            print(f"[OVERSEAS GROUP {group_idx+1}] Using HYBRID search on source: {target_source}")
            all_results = self.search_hybrid(query, max_results=max_results*3, source_filter=[target_source])
        else:
            print(f"[OVERSEAS GROUP {group_idx+1}] Using KEYWORD search on source: {target_source}")
